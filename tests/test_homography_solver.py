...
def test_calibrate_recovers_screen_coords_single_frame():
    ...
    result = calibrate_box_position(labels_df, screen_w_px=SCREEN_W, screen_h_px=SCREEN_H)
...
def test_fit_recovers_true_H():
    ...
    result = fit_per_frame_homography(
        labels_df, _BOX_BL_SCREEN, _BOX_BR_SCREEN,
...
def test_behavior_to_screen():
    ...
    sx, sy = behavior_to_screen(0.0, MY, screen_w_px=SCREEN_W, screen_h_px=SCREEN_H,
                                url_bar_h_px=URL_H, canvas_x_pad_px=X_PAD, max_y_coord=MY)
    assert sy == pytest.approx(SCREEN_H, abs=1.0)
    assert sx == pytest.approx(X_PAD, abs=1e-6)

    # true_x=1 → right canvas edge.
    sx1, _ = behavior_to_screen(1.0, 0.0, screen_w_px=SCREEN_W, screen_h_px=SCREEN_H,
                                url_bar_h_px=URL_H, canvas_x_pad_px=X_PAD, max_y_coord=MY)
    assert sx1 == pytest.approx(SCREEN_W - X_PAD, abs=1e-6)

    # true_y=0 → url_bar_h_px.
    _, sy0 = behavior_to_screen(0.0, 0.0, screen_w_px=SCREEN_W, screen_h_px=SCREEN_H,
                                url_bar_h_px=URL_H, canvas_x_pad_px=X_PAD, max_y_coord=MY)
    assert sy0 == pytest.approx(URL_H, abs=1e-6)

    # Degenerate: url_bar_h_px=0, canvas_x_pad_px=0, max_y_coord=1 → old formula.
    sx, sy = behavior_to_screen(0.5, 0.4, screen_w_px=SCREEN_W, screen_h_px=SCREEN_H,
                                url_bar_h_px=0, canvas_x_pad_px=0, max_y_coord=1.0)
...
def test_big_star_residual_zero_with_true_H():
    ...
    labels_df = pd.DataFrame(
        [_label_row(frame_idx, "big_star", star_fx, star_fy)]
    )
...
def test_big_star_skips_excluded_frame():
    ...
    per_frame_h = pd.DataFrame(
        [
            {
                "frame_idx": frame_idx,
                **{col: float("nan") for col in ["h00","h01","h02","h10","h11","h12","h20","h21","h22"]},
                "n_correspondences": 0,
    ...
    trials_df = pd.DataFrame(
        [{"trial_idx": 1, "tpt": 0, "true_x": 0.5, "true_y": 0.4,
          "video_frame_reveal": frame_idx - 5}]
    )

    result = big_star_residuals(per_frame_h, labels_df, trials_df,
                               url_bar_h_px=0, canvas_x_pad_px=0, max_y_coord=1.0)
    assert result.empty
...
def test_big_star_uses_most_recent_reveal():
    ...
    trials_df = pd.DataFrame(
        [
            {"trial_idx": 1, "tpt": 0, "true_x": 0.1, "true_y": 0.1,
             "video_frame_reveal": 50},
            {"trial_idx": 1, "tpt": 1, "true_x": true_x, "true_y": true_y,
